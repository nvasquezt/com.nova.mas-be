...
export const addNewTrackingLog = async (trackingLog: TrackingLogs) => {
  const newTrackingLog = await prisma.trackingLogs.create({
    data: trackingLog,
  });
  if (newTrackingLog) {
...
export const updateTrackingLog = async (
  idLog: number,
  trackingLog: TrackingLogs
) => {
  const updatedTrackingLog = await prisma.trackingLogs.update({
    where: { idLog },
    data: trackingLog,
  });
...
export const deleteTrackingLog = async (idLog: number) => {
  const deletedTrackingLog = await prisma.trackingLogs.delete({
    where: { idLog },
  });
