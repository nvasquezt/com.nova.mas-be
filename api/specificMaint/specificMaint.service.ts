...
export const getOneSpecificMaint = async (idSpecific: number) => {
  const oneSpecMaint = await prisma.specMaintenances.findUnique({
    where: { idSpecific },
  });
  if (oneSpecMaint) {
    return oneSpecMaint;
...
export const addNewSpecificMaint = async (specificMaint: SpecMaintenances) => {
  const newSpecMaint = await prisma.specMaintenances.create({
    data: specificMaint,
  });
  if (newSpecMaint) {
    return newSpecMaint;
...
export const updateSpecificMaint = async (
  idSpecific: number,
  specificMaint: SpecMaintenances
) => {
  const updatedSpecMaint = await prisma.specMaintenances.update({
    where: { idSpecific },
    data: specificMaint,
  });
  if (updatedSpecMaint) {
...
export const deleteSpecificMaint = async (idSpecific: number) => {
  const deletedSpecMaint = await prisma.specMaintenances.delete({
    where: { idSpecific },
  });
  if (deletedSpecMaint) {
