import { PrismaClient, CtvMaintenances } from '@prisma/client';
  ...
  export const getOneFixMaint = async (idCorrective: number) => {
    const oneFixMaint = await prisma.ctvMaintenances.findUnique({ where: { idCorrective } });
    if (oneFixMaint) {
      return oneFixMaint;
    } else {
      return null;
  ...
  export const updateFixMaint = async (idCorrective: number, fixMaint: CtvMaintenances) => {
    const updatedFixMaint = await prisma.ctvMaintenances.update({ where: { idCorrective }, data: fixMaint });
    if (updatedFixMaint) {
  ...
  export const deleteFixMaint = async (idCorrective: number) => {
    const deletedFixMaint = await prisma.ctvMaintenances.delete({ where: { idCorrective } });
    if (deletedFixMaint) {
